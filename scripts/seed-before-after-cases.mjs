/**
 * One-off seed for before_after_cases after replacing the old face/body/breast
 * categories with 5 named procedures (nose, gynecomastia, arm-lift, cleft-lip,
 * otoplasty) — 3 placeholder cases per category, reusing existing stock image
 * pairs until real patient photos are uploaded via /admin/before-after. Not
 * shipped; run manually via `node scripts/seed-before-after-cases.mjs`.
 */
...
const rows = [
  // Nose Cases
  {
    slug: "nose-1",
    category: "nose",
    before_image: img.F1_BEFORE,
    after_image: img.F1_AFTER,
    order_index: 0,
  ...
  },
  {
    slug: "nose-2",
    category: "nose",
    before_image: img.F2_BEFORE,
    after_image: img.F2_AFTER,
    order_index: 1,
  ...
  },
  {
    slug: "nose-3",
    category: "nose",
    before_image: img.F3_BEFORE,
    after_image: img.F3_AFTER,
    order_index: 2,
  ...
  // Gynecomastia Cases
  {
    ...
    before_image: img.F4_BEFORE,
    after_image: img.F4_AFTER,
    order_index: 0,
  ...
  },
  {
    slug: "gynecomastia-2",
    category: "gynecomastia",
    before_image: img.F5_BEFORE,
    after_image: img.F5_AFTER,
    order_index: 1,
  ...
  },
  {
    slug: "gynecomastia-3",
    category: "gynecomastia",
    before_image: img.F6_BEFORE,
    after_image: img.F6_AFTER,
    order_index: 2,
  ...
  // Arm Lift Cases
  {
    slug: "arm-lift-1",
    category: "arm-lift",
    before_image: img.F7_BEFORE,
    after_image: img.F7_AFTER,
    order_index: 0,
  ...
  },
  {
    slug: "arm-lift-2",
    category: "arm-lift",
    before_image: img.B1_BEFORE,
    after_image: img.B1_AFTER,
    order_index: 1,
  ...
  },
  {
    slug: "arm-lift-3",
    category: "arm-lift",
    before_image: img.B2_BEFORE,
    after_image: img.B2_AFTER,
    order_index: 2,
...
async function main() {
  const { error: delError, count: delCount } = await supabase
    .from("translations")
    .delete({ count: "exact" })
    .like("key", "pages.beforeAfter.groups%");
  if (delError) throw delError;
  console.log("translations: removed", delCount, "stale group-label rows");

  const { error, count: insCount } = await supabase
    .from("before_after_cases")
    .insert(rows, { count: "exact" });
  if (error) throw error;
  console.log("before_after_cases: inserted", insCount, "rows across 5 categories");
}
