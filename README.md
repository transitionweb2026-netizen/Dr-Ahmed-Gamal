...
4. **Social media links are placeholders.** `contactInfo.social` in `src/constants/contactInfo.ts` points at placeholder Instagram/Facebook URLs (matching the legacy mockups' always-present "Connect Socially" card) because no real accounts were provided — swap in the real handles before launch, or clear the object to hide the social row/card again.
